    ...
    for contour in contours:
        if isCircle(contour, contours):
            ...
            if .8 <= aspect_ratio <= 1.1 and area > 1000:
                distance = round(getDist(390, 24, abs(w)), 0)
                cv.rectangle(frame, (x, y), (x + w, y + h), colored_box, 2)
                cv.putText(img, color.upper() + " BALL " + str(distance) + "CM", (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, colored_box, 2)
...
def createRedMask(img):
    ...
    lower_red_1 = np.array([130, 83, 105])
